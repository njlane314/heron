...
apps/config/ -- event column schema references
----------------------------------------

``apps/config/`` stores the default event column TSV consumed by the event
builder. The ``apps/config/event_columns.tsv`` file enumerates event-level
columns and is used automatically unless you pass a custom TSV path to the
CLI.
