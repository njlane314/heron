// macros/render_weighted_background_event_displays.C
//
// Render event displays for background MC events after a score cut.
//
// Important:
...
//   - By default this targets weighted MC backgrounds. If you also want EXT,
//     add a row-wise EXT flag and keep EXT rows alongside !signal MC rows.
//
// Run with:
//   ./heron macro render_weighted_background_event_displays.C
//   ./heron macro render_weighted_background_event_displays.C \
//     'render_weighted_background_event_displays("./scratch/out/event_list.root")'
//
// Examples:
//   ./heron macro render_weighted_background_event_displays.C \
//     'render_weighted_background_event_displays("", "sel_muon", "is_signal", "w_nominal", 6.0, 12, "top")'
//
//   ./heron macro render_weighted_background_event_displays.C \
//     'render_weighted_background_event_displays("", "sel_muon", "is_signal", "w_nominal", 6.0, 12, "sample", "detector", "plots/bkg_score_gt6", "bkg_score_gt6.pdf", 12345ULL)'
                     ...
                     const std::string &label)
{
    ...
    for (const auto &c : cols)
    {
        ...
        {
            std::cerr << "[render_weighted_background_event_displays] missing "
                      << label << " column '" << c << "'.\n";
            ok = false;
        }
    }
...
int render_weighted_background_event_displays(
    ...
    const std::string &out_dir = "plots/bkg_event_displays_score_gt6",
    const std::string &combined_pdf = "bkg_event_displays_score_gt6.pdf",
    unsigned long long rng_seed = 12345ULL)
{
    return heron::macro::run_with_guard("render_weighted_background_event_displays", [&]() -> int {
        ...
        std::cout << "[render_weighted_background_event_displays] input="
                  << input_path << "\n";

        if (!looks_like_event_list_root(input_path))
        {
            std::cerr << "[render_weighted_background_event_displays] input is not an event-list root file: "
                      << input_path << "\n";
            return 1;
        }
        ...
        if (n_events == 0)
        {
            std::cerr << "[render_weighted_background_event_displays] n_events must be > 0\n";
            return 1;
        }
        ...
        {
            std::cerr << "[render_weighted_background_event_displays] missing weight column '"
                      << weight_col << "'.\n";
            return 1;
        }
        ...
        // Pure MC only: background = MC-like excluding EXT and excluding signal.
        ...
        if (n_cand == 0)
        {
            std::cerr << "[render_weighted_background_event_displays] no background MC candidates after"
        ...
        std::cout << "[render_weighted_background_event_displays] candidate background events = "
                  << n_cand << "\n";
        std::cout << "[render_weighted_background_event_displays] candidate weighted yield    = "
                  << sumw << "\n";
        ...
        {
            std::cerr << "[render_weighted_background_event_displays] unknown mode='"
        ...
        if (chosen.empty())
        {
            std::cerr << "[render_weighted_background_event_displays] no events selected for rendering.\n";
            return 1;
        }
        ...
        std::cout << "[render_weighted_background_event_displays] selected events        = "
                  << chosen.size() << "\n";
        std::cout << "[render_weighted_background_event_displays] selected weighted yield = "
                  << chosen_sumw;
        if (sumw > 0.0)
            std::cout << " (" << (100.0 * chosen_sumw / sumw) << "% of candidate weight)";
        std::cout << "\n";
        std::cout << "[render_weighted_background_event_displays] wrote selected event list: "
                  << selected_tsv << "\n";
        std::cout << "[render_weighted_background_event_displays] note: rendered PDF order follows dataframe order, not chosen rank.\n";

        std::cout << "[render_weighted_background_event_displays] chosen events:\n";
        for (std::size_t rank = 0; rank < chosen.size(); ++rank)
        {
            const auto &c = cand[chosen[rank]];
            std::cout << "  [" << (rank + 1) << "]"
        ...
        std::cout << "[render_weighted_background_event_displays] output dir = "
                  << out_dir << "\n";
        if (!combined_pdf.empty())
        {
            std::cout << "[render_weighted_background_event_displays] combined pdf = "
                      << (std::filesystem::path(out_dir) / combined_pdf).string() << "\n";
        }
        std::cout << "[render_weighted_background_event_displays] done\n";
        return 0;
    });
}
