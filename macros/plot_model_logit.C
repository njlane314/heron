...
bool looks_like_event_list_root(const std::string& path) {
  ...
  if (n < 5 || path.substr(n - 5) != ".root") return false;

  std::unique_ptr<TFile> input_file(TFile::Open(path.c_str(), "READ"));
  if (!input_file || input_file->IsZombie()) return false;
...
RocCurve make_roc_curve(const std::vector<float>& scores, const std::vector<int>& labels) {
  RocCurve out;
  if (scores.size() != labels.size() || scores.empty()) return out;
  ...
  if (n_pos == 0 || n_neg == 0) return out;

  std::sort(ranked.begin(), ranked.end(), [](const std::pair<float, int>& a, const std::pair<float, int>& b) {
...
MetricScan scan_thresholds(ROOT::RDF::RNode node_mc, ROOT::RDF::RNode node_ext, const std::string& signal_sel,
                           int n_thresholds, const std::string& score_expr, double thr_min, double thr_max) {
  MetricScan out;
  if (n_thresholds < 2) n_thresholds = 2;
  if (thr_max < thr_min) std::swap(thr_min, thr_max);

  const double signal_total = *(node_mc.Filter(signal_sel).Sum<double>("__w__"));
  if (signal_total <= 0.0) return out;
  ...
  for (int i = 0; i < n_thresholds; ++i) {
    ...
    const std::string pass_expr = "(" + score_expr + " >= " + std::to_string(thr) + ")";
...
void draw_metric_plot(const MetricScan& scan, const std::string& canvas_id, const std::string& title,
                      const std::string& x_title, const std::string& output_stem) {
  if (scan.x.empty()) return;
  ...
  TGraph g_eff(static_cast<int>(scan.x.size()), scan.x.data(), scan.eff.data());
  TGraph g_pur(static_cast<int>(scan.x.size()), scan.x.data(), scan.pur.data());
  TGraph g_effpur(static_cast<int>(scan.x.size()), scan.x.data(), scan.effpur.data());
...
void draw_stack_plots(Plotter& plotter, std::vector<const Entry*>& mc, std::vector<const Entry*>& data, bool include_data) {
...
void draw_roc_plot(ROOT::RDF::RNode auc_node) {
  ...
  if (roc_logit.fpr.empty() || roc_sigmoid.fpr.empty()) {
    std::cerr << "[plot_model_logit] unable to compute ROC/AUC (missing signal/background entries).\n";
    return;
  }
...
void draw_effpur_plots(ROOT::RDF::RNode node_mc, ROOT::RDF::RNode node_ext, const std::string& signal_sel,
                       int n_thresholds, double raw_threshold_min, double raw_threshold_max,
                       const std::string& output_stem) {
  const MetricScan scan_sigmoid =
      scan_thresholds(node_mc, node_ext, signal_sel, n_thresholds, "inf_score_0_sigmoid", 0.0, 1.0);
  if (scan_sigmoid.x.empty()) {
    std::cerr << "[plot_model_logit] signal denominator is <= 0 for signal_sel='" << signal_sel << "'.\n";
    return;
  }

  std::cout << "[plot_model_logit] best sigmoid threshold=" << scan_sigmoid.best_thr
            << " with efficiency x purity=" << scan_sigmoid.best_effpur << "\n";
  draw_metric_plot(scan_sigmoid, "c_first_inf_score_effpur", "First inference-score threshold scan",
                   "Sigmoid(inference score [0]) threshold", output_stem);

  const MetricScan scan_raw = scan_thresholds(node_mc, node_ext, signal_sel, n_thresholds, "inf_score_0",
                                              raw_threshold_min, raw_threshold_max);
  std::cout << "[plot_model_logit] best raw threshold=" << scan_raw.best_thr
            << " with efficiency x purity=" << scan_raw.best_effpur << "\n";
  draw_metric_plot(scan_raw, "c_first_inf_score_effpur_raw", "First inference-score raw-threshold scan",
...
int plot_model_logit(const std::string& samples_tsv = "", const std::string& extra_sel = "sel_muon", bool use_logy = true,
  ...
  if (!looks_like_event_list_root(list_path)) {
    std::cerr << "[plot_model_logit] input is not an event list ROOT file: " << list_path << "\n";
    return 2;
  }
  ...
  ROOT::RDF::RNode base =
      rdf.Define("inf_score_0",
                 ...
                 {"inf_scores"})
          .Define("inf_score_0_sigmoid", [](float x) { return 1.0f / (1.0f + std::exp(-x)); }, {"inf_score_0"});

  if (rdf.HasColumn("analysis_channels")) {
    base = base.Define("is_signal_label", [](int analysis_channel) { return (analysis_channel == 15 || analysis_channel == 16) ? 1 : 0; },
                       {"analysis_channels"});
  } else {
    base = base.Define("is_signal_label", [](bool is_signal) { return is_signal ? 1 : 0; }, {"is_signal"});
  }

  ROOT::RDF::RNode node_ext = filter_by_mask(base, mask_ext).Define("__w__", mc_weight);
  ...
  draw_effpur_plots(e_mc.selection.nominal.node, e_ext.selection.nominal.node, signal_sel, n_thresholds,
                    raw_threshold_min, raw_threshold_max, output_stem);

  std::cout << "[plot_model_logit] done\n";
  return 0;
}
