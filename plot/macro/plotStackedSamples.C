// plot/macro/plotStackedSamples.C
//
// Run with:
//   ./nuxsec macro plotStackedSamples.C
//   ./nuxsec macro plotStackedSamples.C 'plotStackedSamples("reco_neutrino_vertex_sce_z")'
//   ./nuxsec macro plotStackedSamples.C 'plotStackedSamples("reco_neutrino_vertex_sce_z","/path/to/samples.tsv",50,0,1200,"w_nominal","true",false)'
//
// Notes:
...
int plot_stacked_samples_impl(const std::string &expr,
                              ...
                              bool use_logy)
{
    ...
    std::cout << "[plotStackedSamples] input=" << list_path << "\n";

    if (!looks_like_event_list_root(list_path))
    {
        std::cerr << "[plotStackedSamples] input is not an event list root file: " << list_path << "\n";
        return 1;
    }

    std::cout << "[plotStackedSamples] mode=event_list\n";
...
int plotStackedSamples(const std::string &expr = "reco_neutrino_vertex_sce_z",
                       const std::string &samples_tsv = "",
                       int nbins = 50,
                       double xmin = 0.0,
                       double xmax = 2.5,
                       ...
                       bool use_logy = false)
{
    return plot_stacked_samples_impl(expr,
                                     samples_tsv,
                                     nbins,
                                     xmin,
                                     xmax,
