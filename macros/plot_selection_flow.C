...
int plot_selection_cut_flow(
    ...
    for (std::size_t i = 0; i <= cut_flags.size(); ++i) {
        CutFlowPoint point;
        point.label = cut_labels[i];
        point.expr = cumulative_expr(cut_flags, i);

        const std::string stage_sel = "(" + point.expr + ")";
        ...
        point.efficiency = signal_pass / signal_total;
        point.purity = (selected_all > 0.0) ? signal_pass / selected_all : 0.0;
        point.mc_purity = (selected_mc > 0.0) ? signal_pass / selected_mc : 0.0;

        points.push_back(point);

        std::cout << i << "\t" << point.label << "\t"
                    << point.efficiency << "\t"
                    << point.purity << "\t"
                    << point.mc_purity << "\n";
    }
