#ifndef PLOT_INFERENCE_SCORE_SYST_SUITE_CXX
#define PLOT_INFERENCE_SCORE_SYST_SUITE_CXX

// macros/plot_inference_score_syst_suite.C
//
...
//
// Run with:
//   ./heron macro plot_inference_score_syst_suite.C
//
// Or:
//   ./heron macro plot_inference_score_syst_suite.C \
//     'plot_inference_score_syst_suite("./scratch/out/event_list_myana.root")'
//
// Example:
//   ./heron macro plot_inference_score_syst_suite.C \
//     'plot_inference_score_syst_suite("./scratch/out/event_list_myana.root", "sel_muon", "is_signal", "w_nominal", 60, -15, 15, "score_systs", true)'
                       ...
                       const std::string &out_stem)
{
    ...
    c.SaveAs(out.c_str());
    std::cout << "[plot_inference_score_syst_suite] saved: " << out << "\n";
}
...
int plotInferenceScoreSystSuite(const std::string &event_list_path = "",
                                  ...
                                  double xmin = -15.0,
                                  double xmax = 15.0,
                                  const std::string &output_stem = "inference_score_syst_suite",
                                  bool make_knob_plots = true)
{
    return heron::macro::run_with_guard("plotInferenceScoreSystSuite", [&]() -> int {
        if (nbins < 1)
            nbins = 1;
        if (xmax < xmin)
            std::swap(xmin, xmax);
        ...
        std::cout << "[plot_inference_score_syst_suite] input=" << input_path << "\n";

        if (!looks_like_event_list_root(input_path))
        {
            std::cerr << "[plot_inference_score_syst_suite] input is not an event-list root file: " << input_path << "\n";
            return 1;
        }
        ...
        {
            std::cerr << "[plot_inference_score_syst_suite] missing weight column/expression: '" << mc_weight << "'\n";
            return 1;
        }
        ...
        if (sources.empty())
        {
            std::cerr << "[plot_inference_score_syst_suite] no direct multisim vector branches were found.\n";
        ...
        for (const auto &src : sources)
        {
            ...
            if (acc.nuniv <= 0)
            {
                std::cout << "[plot_inference_score_syst_suite] source '" << src.label
        ...
        if (make_knob_plots && has_column(node, "weightsGenieUp") && has_column(node, "weightsGenieDn"))
        {
            ...
            {
                std::cout << "[plot_inference_score_syst_suite] GENIE up/down knob vectors were present but empty.\n";
            }
        }
        else
        {
            std::cout << "[plot_inference_score_syst_suite] skipping explicit GENIE knob plots.\n";
        }

        std::cout << "[plot_inference_score_syst_suite] done\n";
        return 0;
    });
}

int plot_inference_score_syst_suite(const std::string &event_list_path = "",
                                     const std::string &base_sel = "sel_muon",
                                     const std::string &signal_sel = "is_signal",
                                     const std::string &mc_weight = "w_nominal",
                                     int nbins = 60,
                                     ...
                                     bool make_knob_plots = true)
{
    return plotInferenceScoreSystSuite(event_list_path,
                                         base_sel,
                                         signal_sel,
                                         mc_weight,
                                         nbins,
