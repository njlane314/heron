...
void draw_stack_plots(Plotter &plotter, std::vector<const Entry *> &mc,
    ...
    stage_log("drawing stack with " +
              std::to_string(std::max(
                  0, static_cast<int>(adaptive_edges.size()) - 1)) +
              " bins");
    if (include_data)
...
std::vector<double> make_adaptive_score_bins(
    ...
    auto h_bkg_mc_w =
        node_mc_tagged.Filter([](bool is_sig) { return !is_sig; },
                              {"__is_sig_bin__"})
    ...
    stage_log("adaptive binning: filling fine histograms");
    const double total_sw = h_sig_w->Integral(1, n_fine_bins);
    const double total_bw =
        h_bkg_mc_w->Integral(1, n_fine_bins) +
        h_ext_w->Integral(1, n_fine_bins);
    stage_log("adaptive binning: total weighted yields"
              " signal=" + std::to_string(total_sw) +
              ", background=" + std::to_string(total_bw));

    auto normalise_edges = [&](std::vector<double> out) {
        std::sort(out.begin(), out.end());
        out.erase(std::unique(out.begin(), out.end()), out.end());
        if (out.empty() || out.front() > score_lo)
            out.insert(out.begin(), score_lo);
        if (out.back() < score_hi)
            out.push_back(score_hi);
        return out;
    };

    auto build_edges = [&](double min_sig, double min_bkg) {
        std::vector<double> out;
        out.reserve(static_cast<size_t>(max_bins) + 1);
        out.push_back(score_lo);
        ...
        for (int bin = n_fine_bins; bin >= 1; --bin) {
            ...
            const bool pass = (acc_sw >= min_sig && acc_bw >= min_bkg);
            const int n_splits = static_cast<int>(out.size()) - 1;
            const bool can_add_split = (n_splits < max_bins - 1);

            if (bin == 1 || (pass && can_add_split)) {
                const double edge =
                    score_lo + static_cast<double>(bin - 1) * width;
                debug_log("adaptive binning: close coarse bin at edge=" +
                          std::to_string(edge) +
                          " with signal=" + std::to_string(acc_sw) +
                          ", background=" + std::to_string(acc_bw));
                out.push_back(edge);
                acc_sw = 0.0;
                acc_bw = 0.0;
            }
        ...
        return normalise_edges(std::move(out));
    };

    edges = build_edges(nmin_signal, nmin_background);

    if (edges.size() <= 2 && max_bins > 1) {
        const double fallback_sig =
            std::min(nmin_signal,
                     std::max(1.0, total_sw / static_cast<double>(max_bins)));
        const double fallback_bkg =
            std::min(nmin_background,
                     std::max(1.0, total_bw / static_cast<double>(max_bins)));

        if (fallback_sig < nmin_signal || fallback_bkg < nmin_background) {
            stage_log("adaptive binning: requested thresholds collapse to one bin;"
                      " retrying with compatible thresholds"
                      " signal=" + std::to_string(fallback_sig) +
                      ", background=" + std::to_string(fallback_bkg));
            edges = build_edges(fallback_sig, fallback_bkg);
        }
    }
    ...
    stage_log("adaptive binning: built " +
              ...
              " bins");
    if (edges.size() <= 2) {
        stage_log("adaptive binning: warning: still only one bin;"
                  " lower nmin_signal/nmin_background if this is not desired");
    }
...
int plot_model_logit_adaptive_binning(
    ...
    if (has_analysis_channels) {
        ...
        stage_log("warning: neither analysis_channels nor is_signal exists;"
                  " defaulting is_signal_label=false");
    ...
    if (!extra_sel.empty()) {
        ...
        if (named_column) {
            selected = selected.Filter([](bool pass) { return pass; }, {extra_sel});
        } else {
    ...
    ROOT::RDF::RNode node_data = filter_by_sample_mask(selected, mask_data);
    ...
    const std::string adaptive_signal_sel =
        (signal_sel == "is_signal" && !has_is_signal)
            ? "is_signal_label"
            : signal_sel;
    stage_log("adaptive signal selector: '" + adaptive_signal_sel + "'");
